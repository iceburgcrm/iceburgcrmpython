...
const page = usePage();
const allProps = ref(page.props);  // Ensure it's reactive by wrapping in ref if necessary

const props = defineProps({
   test: [Object, Array, null],
   auth: [Object, Array, null]
});

const logo = computed(() => props.auth.system_settings.logo);
const backgroundStyle = computed(() => {
    const imageUrl = logo.value
        ? `data:image/png;base64,${logo.value}`
        : '/static/images/iceburg.png';
