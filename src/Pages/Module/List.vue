                            ...
                            <table ref="tabs" class="table table-primary w-full border-primary border-solid">
                                ...
                                <tr>
                                    <th>
                                        ...
                                        <select class="dropdown-content menu p-2 shadow bg-base-100 rounded-box text-sm w-24" v-model="action_menu">
                                            ...
                                            <option v-if="props.permissions.export === 1" value="xlsx">Export Excel(XLSX)</option>
                                            <option v-if="props.permissions.export === 1" value="xls">Export Excel Compatible (XLS)</option>
                                            <option v-if="props.permissions.export === 1" value="csv">Export CSV</option>
                                            <option v-if="props.permissions.export === 1" value="tsv">Export TSV</option>
                                            <option v-if="props.permissions.export === 1" value="ods">Export ODS</option>
                                            <option v-if="props.permissions.export === 1" value="html">Export HTML</option>
                                            <option v-if="props.permissions.write === 1" value="delete">Delete</option>
                                        </select>
                                    </th>
                                ...
                                <tr v-for="(row, index) in search_data">
                                    <td class="w-50">
                                        ...
                                        <a class="pl-3" :href="`/module/${props.module.name}/edit/${row[$page.props.module.name + '_row_id']}`">Edit</a>
                                    </td>
                                    <td class="text-lg h-10" v-for="(item, key) in props.display_fields">    
                                        <Display
                                            v-bind:data_value="row[item.module.name + '__' + item.name]"
                                            v-bind:record="row"
                                            v-bind:field="item"
                                            :field_data="props.field_data[0][item.name]"
                                        />
...
const props = defineProps({
   ...
   module: [Object, Array, null],
   field_data: [Object, Array, null],
   records_object: [Object, Array, null],
   records: [Object, Array, null],
   display_fields: [Object, Array, null],
   search_fields: [Object, Array, null],
   order_by_fields: [Object, Array, null],
   page: [Object, Array, null],
   request: [Object, Array, null],
   relationship: [Object, Array, null],
   auth: [Object, Array, null],
   permissions: [Object, Array, null],
});

const per_page = ref(props.auth.system_settings.search_per_page);

const page = ref(1);
const search_order = ref('asc');
const order_by = ref('');
const module_id = ref(props.module.id);
const search_type = ref('module');

const pagination_links = ref(reactive(props.records_object.links));

let search_data = ref(props.records.data);
let fieldValueData = {};
let params = [];
...
const prop_fields = computed(() => props.fields);
const request_values = computed(() => props.request);
...
const arrayFailed = Object.entries(props.display_fields).map((arr) => {
    return arr[1]
}
);

const default_display_fields = ref(props.display_fields);
const display_fields = ref(props.display_fields);


for(let key in props.display_fields){
...
onMounted(() => {
    watch([action_menu], async (val) => {
        if(val+'' === 'delete'){
            ...
            if(ok)
            {
                axios.post('/data/delete/' + props.module.id + '/type/module', selected_records.value,
        ...
        {
            axios.post('/data/download/' + props.module.id + '/' + val, selected_records.value,
                {responseType: 'arraybuffer'})
                .then(response => {
