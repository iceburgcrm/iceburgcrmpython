...
const props = defineProps({
   logs: [Object, Array, null]
});

const type = ref('');
const user = ref('');

const display_logs = computed(() => {
  return props.logs.filter(function(item){
    // Make sure to check if user.id matches, assuming user is a sub-object of item
    return (!type.value || type.value === item.type) && 
           (!user.value || user.value === item.user_id);  // Assuming user_id is directly in the item object
  });
});
